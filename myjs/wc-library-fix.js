...
try {
    var cur = P ? (P.getState() || {}).currentSong : null;
    if (cur) {
        var rows = wrap.querySelectorAll(".wc-song-row");
        songs.forEach(function (s, i) { if (s.file && cur.file && s.file === cur.file && rows[i]) rows[i].classList.add("playing"); });
    }
} catch (e) { }
if (P && P.subscribe && !window.__wcPlSub) {
    window.__wcPlSub = true;
    P.subscribe(function (st) {
        try {
            var rows = wrap.querySelectorAll(".wc-song-row");
            rows.forEach(function (x) { x.classList.remove("playing"); });
            (songs || []).forEach(function (s, i) { if (st.currentSong && s.file === st.currentSong.file && rows[i]) rows[i].classList.add("playing"); });
        } catch (x) { }
    });
}
