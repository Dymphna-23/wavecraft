...
r(function(){
function norm(card){var play=card.querySelector(".play-btn"),like=card.querySelector(".like-btn");
...
if(more&&!more.dataset.wcNew){more.dataset.wcNew="1";
more.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();
var song=norm(card);if(window.wcSongMenu)window.wcSongMenu(song,e,{});},true);}
var opts=card.querySelectorAll(".playlist-option");
if(opts[0]&&!opts[0].dataset.wcNew){opts[0].dataset.wcNew="1";
opts[0].addEventListener("click",function(e){e.preventDefault();e.stopPropagation();
var song=norm(card);if(window.wcOpenAdd)window.wcOpenAdd(song);},true);}
if(opts[1]&&!opts[1].dataset.wcNew){opts[1].dataset.wcNew="1";
opts[1].addEventListener("click",function(e){e.preventDefault();e.stopPropagation();
var song=norm(card);if(window.wcOpenCreate)window.wcOpenCreate(song);},true);}
var like=card.querySelector(".like-btn");
if(like&&!like.dataset.wcNew){like.dataset.wcNew="1";
